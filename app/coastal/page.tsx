...
const CENTRAL_OBJETIVO = "PARQUE FOTOVOLTAICO COASTAL";
...
export default function CoastalPage() {
  ...
  async function cargarDatos() {
    ...
    if (error) {
      console.error("Error cargando Coastal:", error);
      setLoading(false);
      return;
    }
  ...
  const latestSnapshotKey = useMemo(() => {
    if (!coastalRows.length) return "";
    return obtenerClaveSnapshot(coastalRows[0].capturado_en_utc);
  }, [coastalRows]);

  const latestRow = useMemo(() => {
    if (!latestSnapshotKey) return null;
    const fila = coastalRows.find(
      (r) => obtenerClaveSnapshot(r.capturado_en_utc) === latestSnapshotKey
    );
    return fila ?? null;
  }, [coastalRows, latestSnapshotKey]);
      ...
      >
        <div style={{ ...cardStyle(), padding: 24 }}>
          ...
          <p style={{ margin: "6px 0 14px 0", color: COLORS.muted, fontSize: 13 }}>
            Últimos 40 registros de Coastal
          </p>
